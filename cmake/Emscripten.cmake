...
set(vkgsplat_WEB_DIR "${CMAKE_SOURCE_DIR}/web")
set(vkgsplat_COI_WORKER "${vkgsplat_WEB_DIR}/coi-serviceworker.min.js")
set(vkgsplat_SHELL_TEMPLATE "${vkgsplat_WEB_DIR}/shell_template.html.in")
set(vkgsplat_INDEX_TEMPLATE "${vkgsplat_WEB_DIR}/index_template.html.in")
...
if(EMSCRIPTEN)
  ...
  # Set WASM build flag
  set(vkgsplat_WASM_BUILD ON CACHE BOOL "Building for WebAssembly" FORCE)

  # Sanitizers don't work with Emscripten
  foreach(sanitizer ADDRESS LEAK UNDEFINED THREAD MEMORY)
    set(vkgsplat_ENABLE_SANITIZER_${sanitizer} OFF CACHE BOOL "Not supported with Emscripten")
  endforeach()

  # Disable static analysis and strict warnings for Emscripten builds
  foreach(option CLANG_TIDY CPPCHECK WARNINGS_AS_ERRORS)
    set(vkgsplat_ENABLE_${option} OFF CACHE BOOL "Disabled for Emscripten")
  endforeach()
  ...
  set(vkgsplat_WASM_INITIAL_MEMORY "33554432" CACHE STRING
      "Initial WASM memory in bytes (default: 32MB)")
  set(vkgsplat_WASM_PTHREAD_POOL_SIZE "4" CACHE STRING
      "Pthread pool size for WASM builds (default: 4)")
  set(vkgsplat_WASM_ASYNCIFY_STACK_SIZE "65536" CACHE STRING
...
function(vkgsplat_configure_wasm_target target)
  if(EMSCRIPTEN)
    ...
    set_property(GLOBAL APPEND PROPERTY vkgsplat_WASM_TARGETS "${target}")
    set_property(GLOBAL PROPERTY vkgsplat_WASM_TARGET_${target}_TITLE "${WASM_TITLE}")
    set_property(GLOBAL PROPERTY vkgsplat_WASM_TARGET_${target}_DESCRIPTION "${WASM_DESCRIPTION}")
    set_property(GLOBAL PROPERTY vkgsplat_WASM_TARGET_${target}_OUTPUT_NAME "${OUTPUT_NAME}")

    target_compile_definitions(${target} PRIVATE vkgsplat_WASM_BUILD=1)
    ...
    target_link_options(${target} PRIVATE
      ...
      "-sASYNCIFY=1"
      "-sASYNCIFY_STACK_SIZE=${vkgsplat_WASM_ASYNCIFY_STACK_SIZE}"
      # Memory configuration
      "-sALLOW_MEMORY_GROWTH=1"
      "-sINITIAL_MEMORY=${vkgsplat_WASM_INITIAL_MEMORY}"
    ...
    if(EXISTS "${vkgsplat_SHELL_TEMPLATE}")
      configure_file(
        "${vkgsplat_SHELL_TEMPLATE}"
        "${CONFIGURED_SHELL}"
        @ONLY
      )
      ...
      message(FATAL_ERROR "Shell template not found: ${vkgsplat_SHELL_TEMPLATE}")
    endif()

    # Copy service worker to target build directory for standalone target builds
    if(EXISTS "${vkgsplat_COI_WORKER}")
...
function(vkgsplat_create_web_dist)
  if(NOT EMSCRIPTEN)
    return()
  endif()
  ...
  get_property(WASM_TARGETS GLOBAL PROPERTY vkgsplat_WASM_TARGETS)
  ...
  foreach(target ${WASM_TARGETS})
    get_property(TITLE GLOBAL PROPERTY vkgsplat_WASM_TARGET_${target}_TITLE)
    get_property(DESCRIPTION GLOBAL PROPERTY vkgsplat_WASM_TARGET_${target}_DESCRIPTION)
...
")
  ...
  if(EXISTS "${vkgsplat_INDEX_TEMPLATE}")
    configure_file("${vkgsplat_INDEX_TEMPLATE}" "${INDEX_OUTPUT}" @ONLY)
  else()
    message(WARNING "Index template not found: ${vkgsplat_INDEX_TEMPLATE}")
  endif()
  ...
  foreach(target ${WASM_TARGETS})
    ...
    get_property(OUTPUT_NAME GLOBAL PROPERTY vkgsplat_WASM_TARGET_${target}_OUTPUT_NAME)
